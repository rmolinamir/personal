...
const SIDEBAR_CLOSE_KEYBOARD_SHORTCUT = "Escape";
...
function SidebarProvider({
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      // biome-ignore lint/suspicious/noDocumentCookie: This is safe.
  ...
  React.useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      if (!isMobile && open && event.key === SIDEBAR_CLOSE_KEYBOARD_SHORTCUT) {
        setOpen(false);
        return;
      }
      if (
    ...
    >
      {collapsible === "offcanvas" && state === "expanded" && (
        <button
          ...
          className="fixed inset-0 z-20 bg-foreground/0"
      ...
      <div
        ...
        className={cn(
          "fixed inset-y-0 z-30 hidden h-dvh w-(--sidebar-width) transition-[left,right,width] duration-200 ease-linear md:flex",
          side === "left"
